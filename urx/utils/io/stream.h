...
class GroupDataStream {
 public:
  GroupDataStream(std::vector<urx::GroupData>* group_data, H5::Group& h5_group,
  ...
  void append(const std::shared_ptr<RawData>& raw_data, double sequence_timestamp,
 ...
 private:
  ...
  void _extend(H5::Group& group, H5::DataSet& dataset, const std::shared_ptr<RawData>& value) {
    ...
    if (layout == H5D_CHUNKED) {
      ...
      memcpy(static_cast<char*>(field->getBuffer()) + unitary_size * dimension[0],
             value->getBuffer(), unitary_size * value->getSize());
...
class Stream {
 public:
  Stream(const std::string& filename, std::shared_ptr<Dataset> dataset)
      : _dataset(std::move(dataset)),
        _file(filename.data(), std::filesystem::exists(filename) ? H5F_ACC_RDONLY : H5F_ACC_EXCL) {}

  std::shared_ptr<Dataset> getDataset() const { return _dataset; }
  urx::utils::io::ReaderDataset& getReader() { return _reader; }
  urx::utils::io::WriterDataset& getWriter() { return _writer; }

  void saveToFile() { _writer.write(_file, *_dataset); }
  void loadFromFile() { _reader.read(_file, *_dataset); }

  GroupDataStream createGroupData(const std::shared_ptr<Group>& group, DoubleNan timestamp) {
