...
class TestBindings(unittest.TestCase):
    ...
    def testVariantVectorInternalType(self):
        ...
        mapTypeToCheckFunc = {"int16": group_data.checkInt16Type, "int32": group_data.checkInt32Type,
                              "float32": group_data.checkFloat32Type, "float64": group_data.checkFloat64Type,
                              "complexInt16": group_data.checkComplexInt16Type, "complexInt32": group_data.checkComplexInt32Type,
                              "complexFloat32": group_data.checkComplexFloat32Type, "complexFloat64": group_data.checkComplexFloat64Type}

        def checkInternalType(self, type):
            for item in mapTypeToCheckFunc:
                if item == type:
                    self.assertTrue(mapTypeToCheckFunc[item]())
                else:
                    self.assertFalse(mapTypeToCheckFunc[item]())

        self.assertEqual(group_data.raw_data, uff.VecInt16())
        group_data.raw_data = uff.VecFloat64([1., 2., 3., 4.])
        checkInternalType(self, "float64")
    ...
    def testVariantVectorIntComplex(self, testName, uffType, numpyType, internalType):
        ...
        checkInternalType(self, internalType)
