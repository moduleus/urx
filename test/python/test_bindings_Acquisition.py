...
class TestBindingsAcquisition(unittest.TestCase):
    ...
    def testAcq_GroupData_Group(self):
        ...
        self.assertEqual(acq.sound_speed, urx.DoubleNan())
        ...
        self.assertEqual(sound_speed_ref, urx.DoubleNan())
        self.assertNotEqual(sound_speed_ref.value, np.nan)
        # Check assignment
        acq.sound_speed = urx.DoubleNan(10)
        self.assertEqual(sound_speed_ref, acq.sound_speed)
        sound_speed_ref += 12
        self.assertEqual(sound_speed_ref, acq.sound_speed)
        sound_speed_ref -= 12
        ...
        self.assertEqual(timestamp_ref, urx.DoubleNan())
        self.assertNotEqual(timestamp_ref.value, np.nan)
        # Check assignment
        acq.timestamp = urx.DoubleNan(10)
        self.assertEqual(timestamp_ref, acq.timestamp)
        timestamp_ref += 12
        self.assertEqual(timestamp_ref, acq.timestamp)
        timestamp_ref -= 12
        ...
        self.assertRaises(
            IndexError, lambda acq: acq.probes[1], acq)
        self.assertEqual(acq.probes[0].type, urx.ProbeType.RCA)

        # excitations is a pointer vector, thus all modifications are shared
        ex = urx.Excitation()
        ex_2 = urx.Excitation()
        ex_2.pulse_shape = "Hello"
        ex_3 = urx.Excitation(ex_2)
        ...
        g = urx.Group()
        g_2 = urx.Group()
        g_2.sampling_type = urx.SamplingType.RF
        g_3 = urx.Group(g_2)
        acq.groups = [g, g_2]
        acq_2 = urx.Acquisition(acq)
        ...
        g_2.sampling_type = urx.SamplingType.IQ
        self.assertEqual(acq.groups[1], g_2)
        ...
        self.assertRaises(
            IndexError, lambda acq: acq.groups[1], acq)
        self.assertEqual(acq.groups[0].sampling_type, urx.SamplingType.RF)

        # groups_data is a pointer vector, thus all modifications are shared
        gd = urx.GroupData()
        gd_2 = urx.GroupData()
        gd_2.group_timestamp = urx.DoubleNan(42)
        gd_3 = urx.GroupData(gd_2)
        gd_3.group_timestamp = urx.DoubleNan(24)
        ...
        self.assertRaises(
            IndexError, lambda acq: acq.groups_data[1], acq)
        self.assertEqual(acq.groups_data[0].group_timestamp, urx.DoubleNan(42))
