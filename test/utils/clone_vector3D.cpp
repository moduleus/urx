...
TEST_CASE("Clone Vector3D", "[Clone]") {
  ...
  // Shared PTR
  std::shared_ptr<Vector3D<double>> d_vec_shared_ptr = std::make_shared<Vector3D<double>>(d_vec);
  ...
  // Weak PTR
  std::weak_ptr<Vector3D<double>> d_vec_weak_ptr = d_vec_shared_ptr;
  ...
  // Optional
  std::optional<Vector3D<double>> d_vec_opt = d_vec;
  ...
  // Weak PTR expired
  d_vec_weak_ptr = std::weak_ptr<Vector3D<double>>();
