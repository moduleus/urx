...
// From uff v0_5, has been removed: Aperture, Probe::focalLength, TransmitWave::weight
template <typename T>
std::shared_ptr<urx::Dataset> ConvertV0_5(const std::string& filename) {
  static std::unordered_map<urx::v0_5::WaveType, urx::Wave::WaveType> old_to_new_wave_type{
      {urx::v0_5::WaveType::CONVERGING_WAVE, urx::Wave::WaveType::CONVERGING_WAVE},
      {urx::v0_5::WaveType::DIVERGING_WAVE, urx::Wave::WaveType::DIVERGING_WAVE},
      {urx::v0_5::WaveType::PLANE_WAVE, urx::Wave::WaveType::PLANE_WAVE},
      {urx::v0_5::WaveType::CYLINDRICAL_WAVE, urx::Wave::WaveType::CYLINDRICAL_WAVE}};

  urx::v0_5::Reader<T> reader;
  reader.setFileName(filename);
  reader.updateMetadata();
  const std::shared_ptr<urx::v0_5::Dataset<T>> dataset_v0_5 = reader.dataset();
  ...
  for (const auto& probe : dataset_v0_5->channelData().probes()) {
    ...
    if (std::dynamic_pointer_cast<urx::v0_5::LinearArray>(probe)) {
      new_probe->type = urx::Probe::ProbeType::LINEAR;
    } else if (std::dynamic_pointer_cast<urx::v0_5::MatrixArray>(probe)) {
      new_probe->type = urx::Probe::ProbeType::MATRIX;
    } else if (std::dynamic_pointer_cast<urx::v0_5::RcaArray>(probe)) {
    ...
    for (const auto& element : probe->elements()) {
      ...
      if (auto probe_linear = std::dynamic_pointer_cast<urx::v0_5::LinearArray>(probe);
          probe_linear) {
        ...
        new_element_geometry->perimeter.emplace_back(x + probe_linear->pitch() / 2, y - height / 2,
                                                     z);
      } else if (auto probe_rca = std::dynamic_pointer_cast<urx::v0_5::RcaArray>(probe);
                 probe_rca) {
      ...
      } else if (auto probe_matrix = std::dynamic_pointer_cast<urx::v0_5::MatrixArray>(probe);
                 probe_matrix) {
  ...
  for (const auto& wave : dataset_v0_5->channelData().uniqueWaves()) {
  ...
  for (const auto& wave : dataset_v0_5->channelData().uniqueWaves()) {
    ...
    new_wave->channel_mapping.reserve(dataset_v0_5->channelData().uniqueEvents().size());
    new_wave->channel_excitations = std::vector<std::weak_ptr<Excitation>>(
        dataset_v0_5->channelData().uniqueEvents().size(),
        retval->acquisition.excitations[map_wave_to_excitation[wave_i]]);
    for (const auto& event : dataset_v0_5->channelData().uniqueEvents()) {
...

  {
    ...
    group->description = "";
    for (const auto& sequence : dataset_v0_5->channelData().sequence()) {
      const auto& event = sequence.evenement().lock();

      Event new_event;
      new_event.transmit_setup.time_offset = 0;
      new_event.transmit_setup.probe_transform = {};
      auto it_probe = std::find_if(dataset_v0_5->channelData().probes().begin(),
                                   dataset_v0_5->channelData().probes().end(),
                                   [probe = event->transmitSetup().probe().lock()](
                                       const std::shared_ptr<urx::v0_5::Probe>& probe_i) {
                                     return probe.get() == probe_i.get();
                                   });
      new_event.transmit_setup.probe =
          retval->acquisition
              .probes[std::distance(dataset_v0_5->channelData().probes().begin(), it_probe)];

      auto it_wave = std::find_if(dataset_v0_5->channelData().uniqueWaves().begin(),
                                  dataset_v0_5->channelData().uniqueWaves().end(),
                                  [wave = event->transmitSetup().getTransmitWave().wave().lock()](
                                      const std::shared_ptr<urx::v0_5::Wave>& wave_i2) {
                                    return wave.get() == wave_i2.get();
                                  });
      new_event.transmit_setup.wave =
          retval->acquisition
              .waves[std::distance(dataset_v0_5->channelData().uniqueWaves().begin(), it_wave)];

      it_probe = std::find_if(dataset_v0_5->channelData().probes().begin(),
                              dataset_v0_5->channelData().probes().end(),
      ...
      new_event.receive_setup.number_samples = dataset_v0_5->channelData().numberOfSamples();
...

  {
    ...
    const size_t sequences = dataset_v0_5->channelData().numberOfFrames();

    group_data->raw_data =
        std::make_shared<urx::RawDataVector<T>>(std::move(dataset_v0_5->channelData().data()));

    group_data->sequence_timestamps.reserve(sequences);
    const auto opt_rate = dataset_v0_5->channelData().repetitionRate();
