...
inline void CreateOperation(OperationNameAdmitter* admitter, OperationCreator* creator);
...
class OperationCreator {
 public:
   ...
   */
  explicit OperationCreator(OperationNameAdmitter* admitter) { CreateOperation(admitter, this); }
  /** Destructor.
   */
...
class OperationFactory {
 public:
  ...
  static Operation* create(const std::string& name) {
    ...
    return (it == registry()->end()) ? static_cast<Operation*>(NULL) : it->second->create();
  }
...
class Session {
 public:
  ...
  static void destroy(const mxArray* pointer) { destroy(getIntPointer(pointer)); }
  ...
  static T* get(intptr_t id) {
    ...
    if (instance == instances->end())
      mexErrMsgIdAndTxt("mexplus:session:notFound", "Invalid id %d. Did you create?", id);
    return instance->second.get();
  }
  static T* get(const mxArray* pointer) { return get(getIntPointer(pointer)); }
  /** Retrieve a const instance or throw if no instance is found.
   */
  static const T& getConst(intptr_t id) { return *get(id); }
  static const T& getConst(const mxArray* pointer) { return getConst(getIntPointer(pointer)); }
  ...
  static bool exist(const mxArray* pointer) { return exist(getIntPointer(pointer)); }
  /** Clear all session instances.
   */
  static void clear() {
    for (int i = 0; i < getInstances()->size(); ++i) mexUnlock();
    getInstances()->clear();
  }
  /** Get instance map.
   */
 ...
 private:
  ...
  static intptr_t getIntPointer(const mxArray* pointer) {
    if (mxIsEmpty(pointer)) mexErrMsgIdAndTxt("mexplus:session:invalidType", "Id is empty.");
    if (sizeof(intptr_t) == 8 && !mxIsInt64(pointer) && !mxIsUint64(pointer))
      mexErrMsgIdAndTxt("mexplus:session:invalidType", "Invalid id type %s.",
...
#define MEX_DEFINE(name)                                                                 \
  class Operation_##name : public mexplus::Operation {                                   \
   public:                                                                               \
    virtual void operator()(int nlhs, mxArray* plhs[], int nrhs, const mxArray* prhs[]); \
                                                                                         \
   private:                                                                              \
  ...
  const mexplus::OperationCreatorImpl<Operation_##name> Operation_##name::creator_(      \
      Operation_##name::Operation_Admitter, NULL);                                       \
...
#define MEX_DEFINE2(name, admitter)                                                                \
  ...
  const mexplus::OperationCreatorImpl<Operation_##name> Operation_##name::creator_(admitter, tag); \
  void Operation_##name::operator()
...
#define MEX_DISPATCH                                                                               \
  void mexFunction(int nlhs, mxArray* plhs[], int nrhs, const mxArray* prhs[]) {                   \
    ...
    if (nrhs < 1 || !mxIsChar(prhs[0]))                                                            \
      mexErrMsgIdAndTxt("mexplus:dispatch:argumentError", "Invalid argument: missing operation."); \
    std::string operation_name(mxGetChars(prhs[0]),                                                \
    ...
    if (operation.get() == NULL) {                                                                 \
      ...
      mexErrMsgIdAndTxt("mexplus:dispatch:argumentError", "Invalid operation: %s",                 \
