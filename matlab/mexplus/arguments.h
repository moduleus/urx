...
class InputArguments {
 public:
  ...
  void parse(int nrhs, const mxArray* prhs[], bool ignore_multi_signatures = false) {
    if (definitions_.empty()) mexErrMsgIdAndTxt("mexplus:arguments:error", "No format defined.");
    ...
    for (entry = definitions_.begin(); entry != definitions_.end(); ++entry)
      if (!parseDefinition(nrhs, prhs, &entry->second)) delete_positions.push_back(entry);
    for (size_t i = 0; i < delete_positions.size(); ++i) definitions_.erase(delete_positions[i]);
    if (definitions_.empty())
      mexErrMsgIdAndTxt("mexplus:arguments:error",
                        (error_message_.empty()) ? "Invalid arguments." : error_message_.c_str());
  ...
  bool is(const std::string& name) const {
    std::map<std::string, Definition>::const_iterator entry = definitions_.find(name);
  ...
  const mxArray* get(const std::string& option_name) const {
    if (definitions_.empty()) mexErrMsgIdAndTxt("mexplus:arguments:error", "No format defined.");
    const Definition& definition = definitions_.begin()->second;
    OptionMap::const_iterator entry = definition.optionals.find(option_name);
    if (entry == definition.optionals.end())
      mexErrMsgIdAndTxt("mexplus:arguments:error", "Unknown option %s.", option_name.c_str());
    return entry->second;
  }
  /** Get a parsed optional argument.
   */
  ...
  /** Access raw mxArray* pointer.
   */
  const mxArray* operator[](const std::string& option_name) const { return get(option_name); }
 ...
 private:
  ...
  bool parseDefinition(size_t nrhs, const mxArray* prhs[], Definition* definition) {
    ...
    if (nrhs < definition->mandatories.size()) {
      message << "Too few arguments: " << nrhs << " for at least " << definition->mandatories.size()
              << ".";
    ...
    if (nrhs - index == 1 && mxIsStruct(prhs[index]) && mxGetNumberOfElements(prhs[index]) == 1) {
      ...
      for (int field_index = 0; field_index < config.fieldSize(); ++field_index) {
        // Check the config structure for fields.
        option_name = config.fieldName(field_index);
        if (option_name.size() > kMaxOptionNameSize) {
          message << "Option name too long: " << option_name.size() << " characters for "
                  << kMaxOptionNameSize - 1;
          error_message_.assign(message.str());
          return false;
        }
        OptionMap::iterator entry = definition->optionals.find(option_name);
        if (entry == definition->optionals.end()) {
      ...
      while (true) {
        ...
        if (!option_name_array.isChar()) {
          message << "Option name must be char but is given " << option_name_array.className()
                  << ".";
          error_message_.assign(message.str());
          return false;
        }
        ...
        if (entry->second)
          mexWarnMsgIdAndTxt("mexplus:arguments:warning", "Option '%s' appeared more than once.",
                             option_name.c_str());
...
template <typename T>
T InputArguments::get(const std::string& option_name, const T& default_value) const {
  T value;
...
template <typename T>
void InputArguments::get(const std::string& option_name, const T& default_value, T* value) const {
...
class OutputArguments {
 public:
  ...
  OutputArguments(int nlhs, mxArray** plhs, int maximum_size = 1, int mandatory_size = 0)
      : nlhs_(nlhs), plhs_(plhs) {
    if (mandatory_size > nlhs)
      mexErrMsgIdAndTxt("mexplus:arguments:error", "Too few output: %d for %d.", nlhs,
                        mandatory_size);
    if (maximum_size < nlhs)
      mexErrMsgIdAndTxt("mexplus:arguments:error", "Too many output: %d for %d.", nlhs,
                        maximum_size);
  }
  /** Safely assign mxArray to the output.
   */
  void set(size_t index, mxArray* value) {
    if (index < nlhs_) plhs_[index] = value;
  }
  ...
  mxArray* const& operator[](size_t index) const {
    if (index >= nlhs_)
      mexErrMsgIdAndTxt("mexplus:arguments:error", "Output index out of range: %d.", index);
    return plhs_[index];
  }
