...
struct EventData {
  EventData(std::vector<ChannelData> channel_data, uint64_t event_timestamp,
  ...
  inline bool operator==(const EventData& other) const {
  ...
  inline bool operator!=(const EventData& other) const { return !(*this == other); }

  // Data organized as _channel_data[ReceiveSetup::_channel_mapping.size()][ReceiveSetup::_number_samples]
  std::vector<ChannelData> _channel_data;
