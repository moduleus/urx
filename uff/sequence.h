...
class Sequence : public TimeOffsetBase, TriggerBase {
 public:
  // CTOR & DTOR
  Sequence() = delete;
  Sequence(const std::vector<TimedEvent> timed_events, MetadataType time_offset = 0.,
  ...
  inline void setTimedEvents(std::vector<TimedEvent> timed_events) { _timed_events = timed_events; }
